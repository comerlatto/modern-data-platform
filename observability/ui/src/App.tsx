...
import {
  AlertTriangle, ArrowDown, ArrowRight, Braces, Check, ChevronRight, Clock3,
      ...
      {!filtered.length ? <Empty>Nenhum dataset registrado nesta execução.</Empty> : (
        <div className="table-wrap"><table><thead><tr><th>Dataset</th><th>Origem</th><th>MinIO</th><th>Raw</th><th>Staging</th><th>Intermediate</th><th>Analytics</th><th>Power BI</th><th>Linhas</th><th>Status</th></tr></thead>
          <tbody>{filtered.map((row) => {
            ...
            const testStatuses = tests.map((test) => String(test.status || "").toLowerCase());
            const hasQualityFailure = row.status_reason === "quality_failed" || testStatuses.some((testStatus) => ["error", "fail", "failed", "runtime error"].includes(testStatus));
            const hasQualityWarning = row.status_reason === "quality_warning" || testStatuses.some((testStatus) => ["warn", "warning"].includes(testStatus));
            const displayedStatus: Status = hasQualityFailure ? "failed" : hasQualityWarning ? "warning" : row.status;
            const displayedLabel = hasQualityFailure ? "Falha de qualidade" : hasQualityWarning ? "Processado com aviso" : undefined;
            return <Fragment key={row.dataset}><tr className={isExpanded ? "dataset-row dataset-row--expanded" : "dataset-row"}>
              <td><button className="dataset-link" onClick={() => void toggle(row.dataset)} aria-expanded={isExpanded} aria-controls={`dataset-tests-${row.dataset}`}><ChevronRight className={isExpanded ? "is-expanded" : ""} size={16} aria-hidden="true" /><strong>{row.dataset}</strong></button></td>
              ...
              <td>{number(row.raw_row_count)}</td><td><StatusMark status={displayedStatus} label={displayedLabel} /></td>
